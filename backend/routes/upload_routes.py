...
def process_file_background(file_id, stored_name, original_name, file_ext):
    ...
    def _check_timeout():
        ...
        if _time.time() - start_time > MAX_PROCESS_TIME:
            raise Exception(f"处理超时（超过 {MAX_PROCESS_TIME} 秒），文件可能太大或太复杂")

    try:
        update_file_status(file_id, "parsing")
        set_progress(file_id, 100, 0, "开始解析...")
        ...
        _check_timeout()
        set_progress(file_id, 100, 70, "清洗数据...")
        cleaned_text = clean_text(raw_text)
        title = os.path.splitext(original_name)[0]

        _check_timeout()
        set_progress(file_id, 100, 85, "生成 Markdown...")
        markdown_content = text_to_markdown(cleaned_text, title=title)

        # 保存 Markdown
        set_progress(file_id, 100, 95, "保存文件...")
        ...
        set_progress(file_id, 100, 98, "生成树结构...")
        import json
        ...
        print(f"[完成] {original_name} -> {output_path}")

    except Exception as e:
        del_progress(file_id)
        error_msg = str(e)
        print(f"[失败] {original_name}：{error_msg}")
        traceback.print_exc()
    ...
    for file in files:
        ...
        if not is_allowed_file(filename):
            results.append({
                ...
                "message": f"不支持的文件格式，仅支持：PDF、Word、Excel、图片、Markdown"
            })
            continue
...
def download_file(file_id):
    ...
    if file_info["status"] != "done":
        return jsonify({"success": False, "message": f"文件状态是 {file_info['status']}，无法下载"}), 400
...
def delete_files():
    ...
    try:
        ...
        if not file_ids:
            return jsonify({"success": False, "message": "没有指定要删除的文件"}), 400

        delete_files_batch(file_ids)

        return jsonify({"success": True, "message": f"已删除 {len(file_ids)} 个文件"})
    except Exception as e:
        import traceback
        traceback.print_exc()
...
def download_batch():
    ...
    file_ids = data.get("file_ids", [])
    if not file_ids:
        return jsonify({"success": False, "message": "没有选中任何文件"}), 400
    ...
    if not done_files:
        return jsonify({"success": False, "message": "选中的文件中没有已完成的"}), 400
...
def export_pdf(file_id):
    """
    将指定文件的 Markdown 内容转为 PDF 并返回
    使用 fpdf2 + 本地微软雅黑字体在后端渲染，不依赖任何外部系统库或打印机驱动
    """
    try:
        import io
        ...
        if not file_info:
            return jsonify({"success": False, "message": "文件不存在"}), 404
        if file_info["status"] != "done":
            return jsonify({"success": False, "message": "文件尚未解析完成"}), 400

        output_path = os.path.abspath(file_info["output_path"])
        if not os.path.exists(output_path):
            return jsonify({"success": False, "message": "Markdown 文件已被删除"}), 404
        ...
        return jsonify({"success": False, "message": f"缺少依赖库，请先安装：pip install fpdf2\n详细：{str(e)}"}), 500
    except Exception as e:
        return jsonify({"success": False, "message": f"PDF 生成失败：{str(e)}"}), 500
...
def get_parse_progress():
    ...
    for fid in parsing_ids:
        ...
        if p:
            ...
            progress_data[str(fid)] = {"total": 100, "done": 0, "stage": "准备中...", "pct": 0}
...
def get_file_content(file_id):
    ...
    if file_info["status"] != "done":
        return jsonify({"success": False, "message": "文件还未解析完成，无法读取"}), 400

    output_path = file_info["output_path"]
    output_path = os.path.abspath(output_path) if output_path else ""
    if not output_path or not os.path.exists(output_path):
        return jsonify({"success": False, "message": "文件已被删除或路径无效"}), 404
...
def save_file_content(file_id):
    ...
    if file_info["status"] != "done":
        return jsonify({"success": False, "message": "文件还未解析完成，无法保存"}), 400
...
def get_tree(file_id):
    ...
    if not tree_path:
        return jsonify({"success": False, "message": "没有找到树文件"}), 404

    tree_path = os.path.abspath(tree_path)
    if not os.path.exists(tree_path):
        return jsonify({"success": False, "message": "树文件已被删除"}), 404
    ...
    try:
        ...
        return jsonify({"success": False, "message": f"读取树文件失败：{str(e)}"}), 500
